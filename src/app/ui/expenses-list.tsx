...
function CreateExpenseModal({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) {
  ...
  const [highExpenseThreshold, setHighExpenseThreshold] = useState<number | null>(null);
  ...
  useEffect(() => {
    ...
    fetch("/api/settings", { headers: { Accept: "application/json" } })
      .then((r) => r.json())
      .then((body: { data?: { highExpenseThreshold?: string | number | null } }) => {
        if (!isActive) return;
        const t = body.data?.highExpenseThreshold;
        setHighExpenseThreshold(typeof t === "string" || typeof t === "number" ? Number(t) : null);
      })
      .catch(() => {});
    return () => { isActive = false; };
  }, []);
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (highExpenseThreshold !== null && Number(amount) > highExpenseThreshold) {
  ...
  const inputCls = "w-full rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-950 placeholder:text-slate-400 focus:border-slate-500 focus:outline-none";
  ...
  return (
    ...
    {showThresholdConfirm ? (
      <ConfirmHighExpenseModal
        amount={Number(amount)}
        isSubmitting={isSubmitting}
        onCancel={() => setShowThresholdConfirm(false)}
        onConfirm={() => { setShowThresholdConfirm(false); void saveExpense(); }}
        threshold={highExpenseThreshold ?? 0}
      />
    ) : null}
    </>
  );
...
function ConfirmHighExpenseModal({
  amount,
  threshold,
  isSubmitting,
  onCancel,
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/50 p-4" onClick={onCancel}>
      <div className="w-full max-w-sm rounded-2xl bg-white shadow-xl" onClick={(e) => e.stopPropagation()}>
        <div className="border-b border-slate-200 px-6 py-4">
          <h2 className="text-sm font-semibold text-slate-950">Confirmar gasto de alto monto</h2>
        </div>
        <div className="px-6 py-5">
          <p className="text-sm text-slate-700">
            Este gasto de {formatMoney(amount)} supera el umbral configurado de {formatMoney(threshold)}. ¿Confirmás que querés registrarlo?
          </p>
        </div>
        <div className="flex justify-end gap-3 border-t border-slate-200 px-6 py-4">
          <button className="rounded-lg px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100" disabled={isSubmitting} onClick={onCancel} type="button">Cancelar</button>
          <button className="rounded-lg bg-violet-600 px-4 py-2 text-sm font-medium text-white hover:bg-violet-700 disabled:opacity-50" disabled={isSubmitting} onClick={onConfirm} type="button">
