  ...
  return (
    <tr className="hover:bg-slate-50/50">
      <td className="px-4 py-3">
        <span className="rounded-lg bg-slate-100 px-2 py-0.5 font-mono text-xs font-medium text-slate-600">
          {service.code}
        </span>
      </td>
        ...
        <form className="space-y-4 px-6 py-5" onSubmit={handleSubmit}>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                className="w-full rounded-lg border border-slate-300 py-2 pl-7 pr-3 text-sm text-slate-950 focus:border-slate-500 focus:outline-none"
                disabled={isSubmitting}
                id="svc-price"
                inputMode="decimal"
                min="0.01"
...
function StatCard({
  ...
  return (
    <div className={`rounded-xl border px-4 py-3 ${colors[accent]}`}>
      ...
      <p className={`mt-0.5 text-2xl font-semibold tracking-tight ${colors[accent].split(" ")[2]}`}>
        {value}
      </p>
    </div>
  );
