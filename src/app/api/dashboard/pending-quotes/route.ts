...
export async function GET() {
  ...
  try {
    const quotes = await getExpiringQuotes(tenantId, 24 * 30);
    return successResponse(quotes.slice(0, 5));
  } catch {
    return errorResponse("No se pudieron cargar las cotizaciones pendientes.");
  }
}
