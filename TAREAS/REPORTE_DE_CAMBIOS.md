...
## Tarea 052 — Edición masiva: cambiar precio, categoría o IVA en múltiples productos a la vez — 2026-07-09
**Estado:** ✅ Completada
**Archivos modificados:** src/app/ui/products-inventory.tsx, src/app/api/products/bulk/route.ts (nuevo)
**Cambios realizados:** En `products-inventory.tsx` (listado principal de productos) se agregó selección múltiple: el checkbox del header ("Seleccionar todos") ahora refleja/alterna la selección de los productos de la página actual (`paginatedProducts`) y cada `ProductRow` tiene su propio checkbox controlado (nuevos props `isSelected`/`onToggleSelect`), con el estado guardado en un `Set<string>` (`selectedIds`). Cuando hay al menos un producto seleccionado aparece `BulkActionsBar` (componente nuevo) arriba de la tabla, con tres acciones independientes: cambiar categoría (selector con `selectableCategories`, ya usado por los modales de crear/editar producto), cambiar alícuota de IVA (selector con `IVA_RATES` importado directamente de `product-validation.ts`, no valores hardcodeados) y ajustar el precio de venta por porcentaje o por monto fijo. Cada botón "Aplicar" arma un `BulkAction` y abre `BulkConfirmModal` (componente nuevo) con el resumen ("Vas a cambiar la categoría de N productos a...", "Vas a ajustar el precio de venta de N productos en +10%...", etc.) y botones Cancelar/Confirmar. Al confirmar, `handleBulkApply` llama a `PATCH /api/products/bulk` con `{ ids, categoryName? | ivaRate? | priceAdjustment? }`, y en éxito refresca el listado, limpia la selección y muestra el mensaje de éxito ya existente (`showSuccess`); en error muestra el mensaje dentro del propio modal sin cerrarlo. Se creó el endpoint nuevo `PATCH /api/products/bulk` (no se reutilizó el `PUT /api/products/[id]` existente porque el ajuste de precio es relativo al precio actual de cada producto, que debe calcularse en el backend a partir de la base de datos y no confiar en un valor final enviado por el cliente): valida `ids` (array no vacío), valida `priceAdjustment` (`mode: "percent" | "fixed"`, `value` numérico finito), exige que tenantId/rol (`OWNER`/`INVENTORY`) sea válido igual que el resto de rutas de productos, y dentro de una única `prisma.$transaction` busca los productos por `id IN (...)` + `tenantId`, calcula el nuevo `salePrice` por producto cuando corresponde (redondeado a 2 decimales), arma el patch por producto y lo pasa por `validateUpdateProductInput` ya existente (sin duplicar reglas de validación) antes de cada `tx.product.update` scopeado por `tenantId`. Se registra un `logAudit` con la acción existente `PRODUCT_UPDATED` (metadata `{ name, bulk: true }`) por cada producto actualizado, sin agregar una acción nueva al enum de auditoría.
**Notas:** No se modificó el schema de Prisma. No se tocó `PUT /api/products/[id]` ni `POST /api/products`. La selección no persiste entre cambios de filtro/página más allá de lo que ya cubre el `Set` (se limpia explícitamente con "Cancelar selección" o tras aplicar un cambio con éxito). `npm run build`, lint y typecheck ejecutados sin errores. `npm test`: 197 pasaron, única falla preexistente y ya documentada (`sales/route.integration.test.ts` — "creates a credit sale with debt through the API flow"); sin fallas nuevas.
---
