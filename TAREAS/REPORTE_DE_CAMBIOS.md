...
UI-02 (mejora de UI pedida por Diego vía `TAREAS/UI_02_historial_devoluciones.md`) — 2026-07-17. Se agregó buscador, filtro de motivo, resumen y badge de motivo a la pestaña "Historial" de Devoluciones, y se reemplazó el folio fabricado (fragmento del CUID) por el folio real de la venta.

- `src/modules/returns/index.ts`: `ReturnListRecord["sale"]` ganó `folio: number`. `listReturns()` ahora acepta `reasonCategory?: ReturnReasonCategory` y `search?: string`; `search` matchea `sale.folio` (si el input es numérico) y/o `sale.customer.name` (contains, case-insensitive) vía `OR`, anidado dentro del `where.sale` existente para que el scope de `tenantId`/`sellerId` siempre aplique con AND independientemente del texto buscado — mismo patrón que `listSales` en `sale-data-access.ts`. `getReturnById()` también expone `sale.folio`.
- `src/app/api/returns/route.ts`: `GET` lee `reasonCategory` y `search` de la query string, valida `reasonCategory` contra `RETURN_REASON_CATEGORIES` (400 si es inválido) y los pasa a `listReturns`.
- `src/app/ui/returns.tsx` (`ReturnHistoryPanel`): se agregó input de búsqueda (folio o cliente) con debounce de 300ms (mismo patrón que `SalesList`) y select de motivo (reusa `RETURN_REASON_OPTIONS`, ambos resetean `page` a 1 al cambiar); resumen de 2 chips arriba de la lista (cantidad de devoluciones del período vía `pagination.total`, y monto devuelto); badge de motivo por color (`REASON_BADGE_STYLES`, ámbar para `DEFECTO`, gris para el resto) en cada fila; `Venta #{record.sale.id.slice(-8).toUpperCase()}` reemplazado por `Venta #{record.sale.folio}` (también en `exportReturnsToCsv`, columna "Venta origen"). No se tocó el resto del CSV ni se agregó panel de "filtros avanzados" — los nuevos controles viven en la fila de filtros existente. La lista de historial sigue siendo cards/filas, no se convirtió en tabla de columnas fijas.

**Resolución del resumen de monto (página vs. período completo)**: `listReturns` no trae un agregado del monto total del período completo — solo `total` (cantidad de registros vía `count()`) y la página actual de resultados. Agregar un `aggregate()` nuevo para el monto period-wide no estaba pedido y hubiera sido una ruta nueva no solicitada por la tarea, así que se optó por sumar `totalAmount` de los `records` ya cargados en la página actual (`pageTotal`, calculado en el cliente con `reduce`). El chip lo etiqueta explícitamente **"devueltos en esta página"** (no "del período") para no inducir a error — cumple la restricción #4 de la tarea de que el resumen debe ser honesto.

**Validación**: `npm run typecheck` y `npm run lint` sin errores. `npm test`: 235 passed / 2 skipped (237), sin regresiones (incluye `src/modules/returns/index.integration.test.ts` y `src/app/api/returns/route.test.ts`, ambos sin cambios necesarios ya que mockean `listReturns` completo o no ejercitan `GET`). Commit `9ac8003`.

**Restricciones respetadas**: CSV no tocado salvo la columna de folio ya existente (ahora con el valor real); sin panel de filtros avanzados nuevo; estilo de fila/card preservado; resumen etiquetado como page-only, no period-wide.
