...
| Sin Nota de Crédito AFIP para devoluciones de ventas ya facturadas | `src/app/ui/return-credit-note-pdf.tsx` (solo comprobante interno, sin llamada a `src/lib/arca`/WSFE) | Encontrado por INGENIERODETESTEO (31-08-2026, reconfirmado 02-09-2026). Una devolución de una venta con CAE real no genera ninguna Nota de Crédito fiscal ante AFIP — gap de cumplimiento para un negocio en producción que ya factura. No es un fix chico, es una feature completa (nuevo comprobante WSFE). Ver `PENDIENTES.md` → `ARCA-NC-01`. |
...
- `Return.refundMethod String?` — nullable solo si la venta original fue `CREDIT` (desde FIX-07). **No se valida contra `Sale.paymentDetails`** — ver `PENDIENTES.md` → `RET-DEV-METODO`.
